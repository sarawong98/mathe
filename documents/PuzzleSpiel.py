...
class RoboticArm:
    ...
    # Hilfsvariabeln für die 3 Gelenke
    self.initial_guess = [0, 0, 0]
    self.result = [0, 0, 0]

    # Abstände zur Zielposition
    self.dx = 0
    self.dy = 0
...
if self.num_joints == 1:
    ...
    self.shoulder_angle = np.arctan2(self.dy, self.dx)
...
# Fall: 2 Gelenke
    ...
    self.shoulder_angle = np.arctan2(self.dy, self.dx) - np.arctan2(self.arm_lengths[1] * np.sin(self.elbow_angle), self.arm_lengths[0] + self.arm_lengths[1] * np.cos(self.elbow_angle))
...
def equations(self, angles):
    ...
    return [self.arm_lengths[0] * np.cos(total_alpha) + self.arm_lengths[1] * np.cos(total_beta) + self.arm_lengths[2] * np.cos(total_gamma) - self.dx, self.arm_lengths[0] * np.sin(total_alpha) + self.arm_lengths[1] * np.sin(total_beta) + self.arm_lengths[2] * np.sin(total_gamma) - self.dy]
...
def update_end_effector(self):
    if self.num_joints == 1:
        ...
        self.elbow_pos = self.shoulder_pos + np.array([self.arm_lengths[0] * np.cos(self.shoulder_angle), self.arm_lengths[0] * np.sin(self.shoulder_angle)])
        self.end_effector_pos = self.elbow_pos + np.array([self.arm_lengths[1] * np.cos(self.shoulder_angle + self.elbow_angle), self.arm_lengths[1] * np.sin(self.shoulder_angle + self.elbow_angle)])
    elif self.num_joints == 3:
        self.elbow_pos = self.shoulder_pos + np.array([self.arm_lengths[0] * np.cos(self.shoulder_angle), self.arm_lengths[0] * np.sin(self.shoulder_angle)])
...
# Initialisiert die Werte für Drag and Drop
...
# Aktualisiert den Roboterarm nach dem Ändern des ersten Arms
def update_arm_length1(self, value):
    self.robotic_arm.set_arm_length(0, int(value))
    self.draw_robotic_arm()

# Aktualisiert den Roboterarm nach dem Ändern des zweiten Arms
def update_arm_length2(self, value):
    self.robotic_arm.set_arm_length(1, int(value))
    self.draw_robotic_arm()

# Aktualisiert den Roboterarm nach dem Ändern des dritten Arms
...
def create_puzzle_pieces(self):
    ...
    random.shuffle(positions) # Zufällige Anordnung der Puzzleteile
    for i in range(2):
        for j in range(2):
...
# Legende oben links für die Steuerung
...
# Definiert das Feld in dem der Endeffektor ausgewählt werden kann
...
def draw_robotic_arm(self):
    ...
    self.elbow_pos = self.shoulder_pos + np.array([self.robotic_arm.arm_lengths[0] * np.cos(self.robotic_arm.shoulder_angle), self.robotic_arm.arm_lengths[0] * np.sin(self.robotic_arm.shoulder_angle)])
    if self.robotic_arm.num_joints == 1:
        self.end_effector_pos = self.elbow_pos
    elif self.robotic_arm.num_joints == 2:
        self.end_effector_pos = self.elbow_pos + np.array([self.robotic_arm.arm_lengths[1] * np.cos(self.robotic_arm.shoulder_angle + self.robotic_arm.elbow_angle), self.robotic_arm.arm_lengths[1] * np.sin(self.robotic_arm.shoulder_angle + self.robotic_arm.elbow_angle)])
    elif self.robotic_arm.num_joints == 3:
        # Zeichnet den Roboterarm auf der Leinwand
        self.wrist_pos = self.elbow_pos + np.array([self.robotic_arm.arm_lengths[1] * np.cos(self.robotic_arm.shoulder_angle + self.robotic_arm.elbow_angle), self.robotic_arm.arm_lengths[1] * np.sin(self.robotic_arm.shoulder_angle + self.robotic_arm.elbow_angle)])
        self.end_effector_pos = self.wrist_pos + np.array([self.robotic_arm.arm_lengths[2] * np.cos(self.robotic_arm.shoulder_angle + self.robotic_arm.elbow_angle + self.robotic_arm.wrist_angle), self.robotic_arm.arm_lengths[2] * np.sin(self.robotic_arm.shoulder_angle + self.robotic_arm.elbow_angle + self.robotic_arm.wrist_angle)])

    # Zeichnet den ersten Arms
...
# Überprüft ob das Puzzleteil im Grid liegt
